...
public class ShoppingCart {
    ...
    public static void initializeShoppingCart() {
        ...
        cartList=ExcelAction.readGoodsFromExcel("cart.xlsx");//表格不空则直接读出
        if(cartList==null || cartList.isEmpty()){//如果该表格读出为空，将预设的信息导入cartList和表格
            ...
            ExcelAction.writeGoodsToExcel(cartList, "cart.xlsx");
        }
    }
    ...
    public static void shoppingCart(){//购物车
        ...
        while(isContinue){
            ...
            System.out.println("列出所有（1），增加（2），减少（3），删除商品（4），计算总价（5），清空购物车（6），查看购物历史信息（7），返回用户操作菜单（8）");
            ...
            try{
                switch(chance){
                        ...
                        case 7:
                            ...
                            ExcelAction.writeGoodsToExcel(shoppingList, "shoppingHistory.xlsx");
                            break;
                        case 8:
                            isContinue=false;
                            break;
